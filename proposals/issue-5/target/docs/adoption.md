...
U `multi-repo` musí navíc agent spuštěný v libovolném participating repository bez domýšlení zjistit, který repository je control plane, kde leží autoritativní work item a Human decisions, které implementation repositories jsou v daném work itemu zapojené a jaké PRs/change candidates tvoří aktuální product-level candidate.
