...
function assignmentProjection(assignment, issuedFromStateVersion, trustBinding = null) {
  ...
  return {
    ...
    ...(trustBinding || assignment.trust_binding
      ? { trust_binding: structuredClone(trustBinding ?? assignment.trust_binding) }
      : {}),
...
function initialProjection(action, runtimeVersion, trustBinding = null) {
  ...
  return {
    ...
    assignment: assignmentProjection(action.assignment, Math.max(0, version - 1), trustBinding),
...
function generateFollowupAssignment(state, runtime, role, purpose, issuedAt) {
  const assignment = generateAssignment({
    ...
    context_entrypoints: [
      { kind: "work_item", ref: state.work_item.control_repository + "#" + state.work_item.issue_number },
      {
        ...
        ref: runtime.trustBinding
          ? runtime.trustBinding.control_plane_repository + "@" + runtime.trustBinding.control_plane_sha + ":" + runtime.projectControlRef
          : ".agenti/project-profile.json"
      },
      { kind: "agent_entrypoint", ref: "AGENTS.md@" + (runtime.trustBinding?.product_bootstrap_sha ?? "local-fixture") }
    ],
    relevant_inputs: {
  ...
  state.assignment = assignmentProjection(assignment, Math.max(0, state.state_version - 1), runtime.trustBinding);
  return assignment;
}
...
export async function processIssue(issueNumber) {
  ...
  const transitionTable = runtime.transitionTable;
  ...
  for (let iteration = 0; iteration < 6; iteration += 1) {
    ...
    if (!state && action.transition_id === "T01") {
      next = initialProjection(action, runtime.projectProfile.runtime_version, runtime.trustBinding);
    } else {
