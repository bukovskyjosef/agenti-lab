...
const controlRoot = resolve(referenceRoot, "..");
...
test("local fixture E2E reaches Done through O/A/D/R/H/P without role relay", async () => {
  ...
  try {
    await mkdir(join(project, ".agenti"), { recursive: true });
    await mkdir(join(project, ".agenti-runtime"), { recursive: true });
    await cp(join(referenceRoot, "schemas"), join(project, ".agenti-runtime", "schemas"), { recursive: true });
    await mkdir(join(project, ".agenti-runtime", "core"), { recursive: true });
    await cp(
      join(referenceRoot, "core", "transitions.json"),
      join(project, ".agenti-runtime", "core", "transitions.json")
    );

    const fixtureProfile = JSON.parse(await readFile(
      join(controlRoot, "profiles", "automated", "single-repo-actions", "project-profile.json"),
      "utf8"
    ));
    fixtureProfile.control_repository = "example/product";
    fixtureProfile.human.principals = [{ actor_id: 1001, login: "human" }];
    await writeFile(
      join(project, ".agenti", "project-profile.json"),
      JSON.stringify(fixtureProfile, null, 2) + "\n"
    );

    const fixtureRuntime = JSON.parse(await readFile(
      join(controlRoot, "profiles", "automated", "single-repo-actions", "runtime.json"),
      "utf8"
    ));
    fixtureRuntime.default_branch = "main";
    await writeFile(
      join(project, ".agenti", "single-repo-actions.json"),
      JSON.stringify(fixtureRuntime, null, 2) + "\n"
    );
    ...
    const runtimeRoot = join(referenceRoot, "profiles", "single-repo-actions", "runtime");
    const stateRuntime = await import(pathToFileURL(join(runtimeRoot, "state.mjs")).href);
    const routingRuntime = await import(pathToFileURL(join(runtimeRoot, "routing-evidence.mjs")).href);
    const runner = await import(pathToFileURL(join(runtimeRoot, "runner.mjs")).href);
    const orchestrator = await import(pathToFileURL(join(runtimeRoot, "orchestrate.mjs")).href);
    ...
    const evidenceObservedAt = new Date(Date.now() - 60_000).toISOString();
    const evidenceValidUntil = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
    ...
    const billingBase = {
      ...
      valid_until: evidenceValidUntil,
    ...
    addNormalized(core.normalizeRoleResult({
      ...
      assignment,
      roleResultSchema: JSON.parse(await readFile(join(referenceRoot, "schemas", "role-result.schema.json"), "utf8"))
    }));
