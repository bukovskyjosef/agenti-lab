...
function issueBody(scenario) {
  ...
  return [
    ...
    "",
    "## Goal", "", goal, "",
    "## Context", "", "Disposable real-GitHub Issue #24 final-profile conformance run.", "",
    "## Scope", "", "Only the requested disposable E2E fixture change.", "",
    "## Out of scope", "", "No unrelated repository changes.", "",
    "## Requirements", "", "- Preserve trusted stable transport and external control-plane binding.", "",
...
async function main() {
  ...
  if (options["human-actor-id"] && Number(options["human-actor-id"]) !== actor.id) {
    throw new Error(
      "Current gh actor " + actor.login + " (" + actor.id + ") differs from --human-actor-id. " +
      "The disposable repository's external project-control profile must configure this exact actor as H."
    );
  }
  console.log(
    "E2E Human actor: " + actor.login + " (" + actor.id + "). " +
    "Runtime project-control authority will independently reject Human commands if this actor is not configured."
  );
