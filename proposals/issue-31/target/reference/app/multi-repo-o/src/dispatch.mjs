...
export function runnerTarget(profile, role, assignment = null) {
  const candidateId = assignment?.execution_route?.runner_candidate_id ?? null;
  if (candidateId) {
    const candidate = profile.execution_routing?.runner_catalog?.[candidateId];
    if (!candidate) throw new Error(`Unknown routed runner candidate ${candidateId}`);
    if (candidate.dispatch?.kind === "GITHUB_WORKFLOW") {
      if (!candidate.dispatch.repository || !candidate.dispatch.workflow) {
        throw new Error(`Routed runner candidate ${candidateId} requires dispatch repository/workflow`);
      }
      return {
        repository: candidate.dispatch.repository,
        workflow: candidate.dispatch.workflow,
  ...
  return {
    ...parseWorkflowAdapterId(runner.adapter_id),
...
export async function dispatchAssignment({ gh, profile, assignment, store, ref = "main" }) {
  const target = runnerTarget(profile, assignment.role, assignment);
