...
const issue29Dir = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(issue29Dir, "..", "..");
const overlay = join(issue29Dir, "target", "reference");
...
async function materializeTree(referenceRoot, { name, sha, prefix }) {
  ...
  if (!listed.length) {
    throw new Error(name + " exact target is empty at " + sha);
  }
  ...
  console.log(
    name + " materialized @ " + sha + " (" + listed.length + " files)"
  );
}
...
const temp = await mkdtemp(join(tmpdir(), "agenti-child29-"));
const referenceRoot = join(temp, "reference");

try {
  await mkdir(referenceRoot, { recursive: true });
  for (const item of refs) {
    await materializeTree(referenceRoot, item);
  }
  await cp(overlay, referenceRoot, { recursive: true, force: true });

  console.log("Child #29 overlay applied from working tree");
  ...
  run(
    ...
    [
      ...
      "doctor",
      "--",
      "./conformance/fixtures/project-profile.routing.valid.json"
    ],
    referenceRoot
  );
  ...
  run(
    process.execPath,
    ["--test", "conformance/routing/static.test.mjs"],
    referenceRoot
  );
  run(
    process.execPath,
    ["--test", "conformance/single-repo-actions/static.test.mjs"],
    referenceRoot
  );
  run(
    process.execPath,
    ["--test", "conformance/single-repo-actions/local-e2e.test.mjs"],
    referenceRoot
  );
  ...
  run(
    "npm",
    ["test"],
    join(referenceRoot, "app", "multi-repo-o")
  );
  ...
  if (process.argv.includes("--docker")) {
    run(
      ...
      [
        "build",
        "-f",
        join(referenceRoot, "app", "multi-repo-o", "Dockerfile"),
        referenceRoot
      ],
      repoRoot
    );
  ...
  console.log("Child #29 combined exact-target validation: OK");
  console.log(
    "Dependencies: " +
    refs.map((item) => item.name + "@" + item.sha).join(", ")
  );
} finally {
